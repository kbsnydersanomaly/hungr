...
3. Load `hungr-project-workflow` always; load `hungr-next-ui-work` for UI; load `hungr-supabase-actions`, `supabase`, and Postgres guidance for schema/action/RLS work. If companions are not mirrored under `.claude/skills/`, read their `.agents/skills/` files directly.
