...
- **Now:** **`P0-E2-design` is accepted** (Kyle-Ben, 2026-07-24, D1–D13 at recommended defaults) and **`P0-E2a` is written and locally green, waiting only on a push.** `.github/workflows/ci.yml` exists with `lint`, `typecheck`, `unit`, and `build`. `P0-E1` is complete (`P0-E1-design`, `P0-E1a`–`P0-E1d`, all 2026-07-24): a clean machine can go from an empty volume to a running app with `pnpm db:bootstrap` then `pnpm env:local`. Next task is `P0-E2b` (database bootstrap / migration check / RLS jobs), which does **not** depend on `P0-E2a` being green.
...
- **CI cannot be marked done without a push.** A workflow is only provable by a green Actions run, and the agent does not push; `P0-E2a` is `[-]` pending a run URL, and `P0-E2b`/`P0-E2c` will land the same way unless the user pushes in-session.
...
- [-] **P0-E2a** Add lint, `tsc --noEmit`, unit/component, and production-build jobs. Depends on: P0-E2-design. Also carries the spec section 5.6 Vitest project split and `.nvmrc`. Progress: 2026-07-24 — written and locally verified in full. `.github/workflows/ci.yml` (jobs `lint`, `typecheck`, `unit`, `build`), `.github/actions/setup/action.yml`, `.nvmrc` (`22`), the `node`/`components`/`rls` Vitest split with `test:ci`/`test:rls`, and the README CI section are all done; every job's local equivalent passes, including `pnpm test:ci` with `.env.local` moved aside and `pnpm build` with only the workflow's placeholder env. Remaining: **the section 7.2 remote proof only** — push the branch, open a pull request, and record the green Actions run URL and per-job durations in the session log; then mark `[x]`. The agent does not push.
