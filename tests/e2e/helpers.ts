...
/**
 * Creates a published menu, a category, and items directly in the database.
 * Use this when the menu-workspace UI is not under test.
 */
export async function seedPublishedMenu(
  restaurantId: string,
  name: string,
  slug: string,
  items: SeedMenuItem[]
  ...
  if (menuError || !menu) {
    throw menuError ?? new Error("Failed to seed menu");
  }

  const { data: category, error: catError } = await admin
    .from("categories")
    .insert({ menu_id: menu.id, name: "Menu" })
    .select()
    .single();

  if (catError || !category) {
    throw catError ?? new Error("Failed to seed category");
  }

  if (items.length > 0) {
    const priceToCents = (price: string) => Math.round(parseFloat(price) * 100);
    const { error: itemsError } = await admin.from("menu_items").insert(
