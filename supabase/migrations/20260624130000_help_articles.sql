...
create index if not exists idx_help_articles_published on help_articles(id) where published = true;
create index if not exists idx_help_categories_sort on help_categories(sort_order);
create index if not exists idx_help_articles_topics on help_articles using gin(topics);
